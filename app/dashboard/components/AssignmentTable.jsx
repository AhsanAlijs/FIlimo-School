...
import Link from "next/link"
  ...
  export function AssignmentTable({assignment,fetchCourses}) {
          ...
          {assignment.length ? assignment.map((item, index) => (
            <TableRow key={index}>
                ...
                <div className='flex items-center text-2xl gap-5 justify-end'>
                    <IoEyeSharp className='text-[#E67D4B] cursor-pointer' onClick={()=>handleDownload(index)}/>
                    <FaPen  onClick={()=>toggleModal(index)} className='text-[#419B41] cursor-pointer'/>
                    <RiDeleteBin6Line className='text-[#FF0000] cursor-pointer'onClick={()=>handleDelete(index)}/>
                </div>
                </TableCell>
          </TableRow>
          )) : <TableRow>
