    ...
    <>
      <div className='max-xl:w-full'>
        <div className='flex shadow-lg  group rounded-2xl flex-col max-md:h-full max-xl:w-full w-[200px] text-center'>
          <div className='image_wrapper p-6 max-md:p-4 rounded-t-2xl ' style={{ backgroundColor: getRandomColor() }}>
            <Image alt='img' src={props?.image?.startsWith("http") ? props.image : `${process.env.NEXT_PUBLIC_BASE_URL}/${props.image}`} width={209} height={209} className='w-[150px] h-[180px] max-sm:h-[160px] rounded-lg shrink-0 object-cover object-top' />
          </div>
          <div className='py-2'>
            <p className="text-gray-700 p-1 max-sm:px-1 text-[15px] max-md:justify-center flex items-center justify-evenly  font-semibold mt-[12px]">
              {props.text.length > 10 ? `${props.text.slice(0, 12)}...` : props.text}
